...
rf_model = load_model(r'/app/models/random_forest_model.pkl')
xgb_model = load_model(r'/app/models/XGBoost.pkl')
gb_model = load_model(r'/app/models/gradient_boosting.pkl')
catboost_model = load_model(r'/app/models/cat_boost.pkl')

# Load the training data for preprocessing
# Update with your dataset path
data = pd.read_csv(r'/app/data/nndb_flat.csv')
...
def home():
    ...
    st.title("Welcome to NUTRIFLEX.Ai!")

    # Introduction
    st.write(
        "In today's fast-paced world, maintaining a balanced diet is essential for overall health and well-being. "
        "NUTRIFLEX.Ai is designed to make personalized nutrition accessible and engaging for everyone."
    )

    # Image
    st.image(r"/app/images/Wearble1.jpg", use_column_width=True)  # Update with your image path
    ...
    st.write(
        "At NutriFLEX.Ai, our mission is to empower individuals to make informed dietary choices through the use of technology. "
    ...
    st.markdown("---")
    st.write("© 2024 NutriFLEX.Ai. All rights reserved.")
...
def about():
    ...
    image = Image.open(r"/app/images/A_professional-looking_portrait_of_a_tech-savvy_cr.webp")
    st.image(image, caption="The Creator behind NUTRIFLEX.Ai", use_column_width=True)

    # Write the About section
    st.write("""
The creator behind **NutriFLEX.Ai** is a passionate innovator with expertise in data science, AI, and health technology. 
...
adapt to their unique needs, helping them achieve their health goals and maintain a balanced lifestyle. **NutriFLEX.Ai** reflects this mission 
...
""")
    # Join Us Section
    st.header("Join Us on Your NutriFLEX.Ai Journey")
    st.write(
        "Whether you're looking to lose weight, gain muscle, or simply eat healthier, NutriFlex.Ai is here to support you every step of the way. "
...
st.sidebar.image(r"/app/images/NUTRIFLEX.png",
